...
def translate_vqa_questions_batch(data: Dict[str, Any]) -> Dict[str, Any]:
    """VQA_annotation의 question_kr 한글 값들을 배치로 번역하여 question_en에 저장"""
    if 'VQA_annotation' in data:
        # 번역할 한글 텍스트들 수집
        korean_texts = []
        text_to_vqa_map = []  # 텍스트와 VQA 항목의 매핑
        
        for vqa_item in data['VQA_annotation']:
            if 'question_kr' in vqa_item and vqa_item['question_kr']:
                korean_texts.append(vqa_item['question_kr'])
                text_to_vqa_map.append(vqa_item)
        
        if not korean_texts:
            print("번역할 한글 VQA 질문이 없습니다.")
            return data
        
        print(f"📝 총 {len(korean_texts)}개 VQA 질문 배치 번역 시작...")
        print(f"번역할 질문들:")
        ...
        # 번역 결과를 각 VQA 항목에 적용
        for i, vqa_item in enumerate(text_to_vqa_map):
            if i < len(english_texts):
                korean_question = vqa_item['question_kr']
                english_question = english_texts[i]
                vqa_item['question_en'] = english_question
                
                if english_question == korean_question:
                    print(f"⚠️ 번역 실패: VQA_{i+1}")
                else:
                    print(f"✅ 번역 완료: VQA_{i+1}")
                    print(f"   질문: {korean_question[:50]}{'...' if len(korean_question) > 50 else ''}")
                    print(f"   번역: {english_question[:50]}{'...' if len(english_question) > 50 else ''}")
        
        print(f"🎉 VQA 배치 번역 완료! {len(korean_texts)}개 질문 처리됨")
    
    return data

def main():
    # 입력 JSON 파일 경로
    input_file = '../data/result/vqa_result.json'
    
    # 출력 JSON 파일 경로
    output_file = '../data/result/vqa_translated_batch.json'
    ...
    translated_data = translate_vqa_questions_batch(data)
    end_time = time.time()
